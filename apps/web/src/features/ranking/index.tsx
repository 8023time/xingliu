...
function RankingPanel({
    ...
    async (cursor: string | null, reset = false) => {
      ...
      try {
        ...
        setItems((currentItems) => {
          const nextItems = reset ? page.items : [...currentItems, ...page.items];
  ...
  useEffect(() => {
    ...
    setHasMore(true);
    setError('');
    void loadRanking(null, true);
  }, [loadRanking]);

  const loadMore = useCallback(() => {
    if (!hasMore || loading || !nextCursor) return;
    void loadRanking(nextCursor);
  }, [hasMore, loadRanking, loading, nextCursor]);
  ...
  const renderItem = useCallback(
        ...
        >
          <span
            className={cn(
              ...
              isTopThree ? theme.rankColors[index] : 'font-sans font-medium text-zinc-300 not-italic',
            )}
          >
    ...
    [theme.activeText, theme.rankColors, type],
  );
