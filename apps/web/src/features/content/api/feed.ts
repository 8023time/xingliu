...
export async function fetchPublicFeed(params: PublicFeedQueryRequest = {}) {
  ...
  const response = await fetch(buildApiUrl(`/api/feed?${searchParams.toString()}`), { cache: 'no-store' });
  if (!response.ok) throw new Error('公开内容流加载失败');

  return ((await response.json()) as ResponseFormat<PublicFeedResponse>).data.items;
}
