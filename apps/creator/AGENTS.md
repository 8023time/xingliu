...
`apps/creator` 是星流创作者中心（B 端），负责创作者的内容生产和发布工作流：
...
- 本文件负责 `apps/creator/` 的项目边界和工程约定。
- 写实现代码前先参考 `docs/rules/FE-CREATOR-RULES.md`。
...
2. 后端执行安全审核（mint-filter → 阿里云 → 决策）和质量评分。
...
```bash
# 开发
pnpm --filter @xingliu/creator dev
